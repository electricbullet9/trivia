...
def get_google_links(page, num_results, q):
    ...
    temp_list = links[:num_results]
    temp_list[-1] = GOOGLE_URL.format(q)
    return temp_list
...
async def search_google(question, num_results):
    ...
    return get_google_links(page, num_results, question)
...
async def multiple_search(questions, num_results):
    ...
    link_list = [get_google_links(page, num_results, questions) for page in pages]
    return link_list
...
def find_nearest(list, value):
    ...
    index = (numpy.abs(arr - value)).argmin()
    difference = numpy.abs(arr[index] - value)
    return difference
